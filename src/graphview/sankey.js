...
const Rect = ({ index, x0, x1, y0, y1, name, value, length, swidth, sheight, colors }) => {
    return (
      <Fragment>
        <rect
          x={x0}
...
class Own extends Component {
  constructor(props) {
    ...
    this.sankeyimpl = d3.sankey().nodeAlign(d3.sankeyJustify).nodeWidth(10).nodePadding(10).extent([[0, 0], [props.width, props.height]]);
  }

  render() {

    if (this.props && this.props.data && this.props.data.nodes.length > 0 && this.props.data.links.length > 0) {
      ...
      return (
        <svg width={width} height={height}>
          <g>{links.map((d, i) => (
                    <Link key={d.index} data={d} width={d.width} length={nodes.length} colors={colors} />
            ))}
          </g>
          <g>{nodes.map((d, i) => (
                  <Rect key={d.index} index={d.index} x0={d.x0} x1={d.x1} y0={d.y0} y1={d.y1} name={d.name} value={d.value} length={nodes.length} swidth={width} sheight={height} colors={colors} />
             ))}
          </g>
        </svg>
      );
